...
public class Joueur : NetworkBehaviour {
	...
	void Update () {
		...
		if (enVie == true) {
			var x = Input.GetAxis ("Horizontal") * 0.1f * vitesseDeplacementActuelle;
			var z = Input.GetAxis ("Vertical") * 0.1f * vitesseDeplacementActuelle;
			...
			if (Input.GetButtonDown ("Fire1")) {
				...
				if (activable) {
					...
					setActivable (false);
					Invoke ("activer", tempsRechargeProjectile);
				}
			}
